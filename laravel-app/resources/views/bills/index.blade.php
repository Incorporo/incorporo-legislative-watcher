...
<div class="space-y-6">
    ...
    <!-- Results Count -->
    <div class="flex items-center justify-between bg-gradient-to-r from-slate-50 to-white px-6 py-4 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center space-x-2 text-sm text-slate-600 font-medium">
            ...
            <span>
                Afișare <span class="font-bold text-slate-900">{{ $bills->firstItem() ?? 0 }}</span> -
                <span class="font-bold text-slate-900">{{ $bills->lastItem() ?? 0 }}</span> din
                <span class="font-bold text-blue-600">{{ $bills->total() }}</span> proiecte
            </span>
        </div>
        <div class="flex items-center space-x-3">
            <label for="sort-select" class="text-sm font-semibold text-slate-700">Sortează:</label>
            <select id="sort-select" onchange="window.location.href = this.value" class="border-slate-300 rounded-lg text-sm font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
